...
每次开发完成可提交的代码后，都需要验证可运行和帮用户自动提交本次更改的 git，具体要求如下：

- 每次提交前应该运行 `./gradlew build` 确保构建通过
- 按仓库已提交的中文分条样例提交
- 每次提交必须包含简短的提交信息，说明本次提交的内容
- 提交信息应使用中文，格式为：`feat: <描述>` 或 `fix: <描述>` 或 `chore: <描述>`
- 提交信息应简洁明了，避免使用过于复杂的描述